...
export const Chat = () => {
  ...
  const handleMoreClick = () => {
    setIsOpen(!isOpen);
    setIsOpens(false); // close the second menu when the first menu is opened/closed
  };
  ...
  const handleMessagesClick = () => {
    setIsOpen(false); // close the entire menu when Messages is clicked
  };

  const handleInputClick = () => {
    setIsOpen(false); // close the entire menu when Input is clicked
  };
  ...
  return (
    <div className="chat">
      <div className="chatInfo">
        <span>
          <img src={data.user?.photoURL} alt="" width={"50px"} style={{ borderRadius: "50%" }} />
          <span>{data.user?.displayName}</span>
        </span>
        <div className="chatIcons">
          <button className='ChatMore' onClick={handleMoreClick}>
            <img src={more} alt="" />
            {isOpen && (
              <div className='more'>
                <li onClick={handleNotificationsClick}>
