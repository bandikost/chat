...
export const Chat = () => {
  ...
  const handleClearMessages = async () => {
    try {
      await deleteDoc(doc(db, "chats", data.chatId)); // delete the entire chat document
    } catch (error) {
          ...
          {isOpen && (
            <div className='more'> 
              ...
              <li><img src={add} alt="" /><span>Создать беседу</span></li>
                <a className='' href="#" onClick={() => setIsOpenClear(!isOpenClear)}>
                  <img src={deleteChat} alt=""  /><span>Удалить чат</span></a>
                  {isOpenClear && (
                        <> 
                 
                          <div className='moreClear'>
                            <li> <img src={Wvolume} alt="" onClick={handleClearMessages} /> Навсегда</li>
                            <li> <img src={Wvolume} alt=""  onClick={handleClearMessages}/> На 10 минут</li>
                            <li> <img src={Wvolume} alt=""  onClick={handleClearMessages}/> На 1 час</li>
                            <li onClick={handleClearMessages}> <img src={Wvolume} alt=""  /> На день</li> </div>
                        </>
                    )}
            </div>
          )}        
